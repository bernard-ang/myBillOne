...
import { getWhatsAppTemplateBodyParams } from '../../../../utils/whatsapp-template';
...
export const makeNameValidator = (
  transactionalEmailApi: TransactionalEmailApi | MultiTemplateTransactionalEmailApi,
  originalName?: string
): AsyncValidatorFn => {
...
@Component({
  selector: 'grabbill-client-multi-template-transactional-email-edit-detail',
  templateUrl: './multi-template-transactional-email-edit-detail.component.html',
  styleUrls: ['./multi-template-transactional-email-edit-detail.component.less'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class MultiTemplateTransactionalEmailEditDetailComponent extends NgxsBaseComponent {
  ...
  constructor(
    ...
    this.form = this.fb.group({
      ...
      transactionalEmailTemplates: this.fb.array([])
    });
  }
  ...
  get transactionalEmailTemplates() {
    return this.form.controls['transactionalEmailTemplates'] as UntypedFormArray;
  }
  ...
  override ngOnInit(): void {
    ...
    this.autoUnsubscribe(
      this.route.params.pipe(
        tap((params: Params) => {
          ...
          if (this.typeId) {
            ...
            this.doAddNewTransactionalEmailTemplate()
          }
        })
      ),
      ...
      this.transactionalEmailType$.pipe(
        tap((transactionalEmailType) => {
          if (!this.isNew && transactionalEmailType) {
            ...
            transactionalEmailType.transactionalEmailTemplates.map(transactionalEmailTemplate => {
              this.doAddTransactionalEmailTemplate(transactionalEmailTemplate)
            })

            this.toggleAutoPurge(transactionalEmailType.autoPurge);

            transactionalEmailType.indexFields.map((indexField) => {
              this.doAddIndexField(indexField, indexField.seqOrder <= 4);
            });
      ...
      this.actions$.pipe(
        ofActionCompleted(NewMultiTemplateTransactionalEmailType),
        switchMap((data: ActionCompletion) => {
          ...
          if (data.result.error) {
            ...
            const transactionalEmail = this.store.selectSnapshot(MultiTemplateTransactionalEmailState.transactionalEmailType)!;
            createMultiTemplateTransactionalEmail(this.store, transactionalEmail, transactionalEmail.name);
          }
      ...
      this.actions$.pipe(
        ofActionCompleted(UpdateMultiTemplateTransactionalEmailType),
        switchMap((data: ActionCompletion) => {
          this.isLoading = false;
          ...
          if (data.result.error) {
            ...
            const transactionalEmail = this.store.selectSnapshot(MultiTemplateTransactionalEmailState.transactionalEmailType);
            this.navigate(['/', 'mt-transactional-email', 'detail', transactionalEmail!.id], { tab: 'settings' });
          }
  ...
  private processWhatsappTemplate(transactionalEmailType: MultiTemplateTransactionalEmailTypeModel) {
  ...
  submitForm(): void {
    ...
    if (this.form.valid) {
      const currentTransactionalEmailTemplates: MultiTemplateTransactionalEmailTemplateModel[] = value.transactionalEmailTemplates;

      for (const transactionalEmailTemplate of currentTransactionalEmailTemplates) {
        const mismatchMergeFields = this.getMismatchMergeFields(transactionalEmailTemplate.emailContent);
        if (mismatchMergeFields.length > 0) {
          this.displayMismatchFieldErrorModal(mismatchMergeFields, transactionalEmailTemplate.emailTemplateName);
          this.isLoading = false;
          return;
        }
      }
      ...
      if (this.isNew) {
        this.store.dispatch(
          new NewMultiTemplateTransactionalEmailType({
            name: value.name,
            code: value.code,
            ...
            transactionalEmailTemplates: currentTransactionalEmailTemplates
          })
        );
      } else {
        this.store.dispatch(
          new UpdateMultiTemplateTransactionalEmailType(this.typeId!, {
            name: value.name,
            code: value.code,
            autoPurge: value.autoPurge,
            autoPurgeByDays: value.autoPurgeByDays,
  ...
  doAddTransactionalEmailTemplate(template: MultiTemplateTransactionalEmailTemplateModel): void {
    ...
    this.transactionalEmailTemplates.push(indexFieldForm);
    this.cd.markForCheck();
  }
  ...
  deleteTemplate(index: number) {
    doDeleteIndex(this.transactionalEmailTemplates, index, this.cd)
  }
}
