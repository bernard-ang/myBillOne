...
export const getUploadExcelData = (file: File, fields: BaseIndexFieldModel[]) => {
  return of(file).pipe(
...
const parseWorkbook = (fields: BaseIndexFieldModel[]) => {
...
const validateColumnHeaders = (fields: BaseIndexFieldModel[], worksheet: WorkSheet): Array<string | null> => {
  ...
  return fields
    ...
    .map((field) => {
      ...
      if (row1Cell && row1Cell.v.trim() === field.header) {
        return null;
      } else {
        return `Expecting [${field.header}] in column ${String.fromCharCode(letterCharCode - 1)}1 but is [${
          (row1Cell && row1Cell.v) || ""
        }]`;
      }
    })
...
const getRowsData = (fields: BaseIndexFieldModel[], worksheet: WorkSheet): Observable<any[]> => {
  let currentRow = 2;
  const rows: any[] = [];
...
const mapRowDataToIndexRow = (fields: BaseIndexFieldModel[]) => {
  return (row: any[]) => {
    return fields
      .filter((field) => field.applicable)
      ...
      .reduce((previousValue, currentValue) => {
        if (currentValue.field.dataType === DataType.NUMBER) {
          ...
          if (currentValue.value) {
            let parsedDate = dateParse(currentValue.value, "M/d/yy", new Date());
            if (isNaN(parsedDate.getTime())) {
              parsedDate = dateParse(currentValue.value, "dd/MM/yyyy", new Date());
            }
            previousValue[getIndexFieldName(currentValue.field)] = parsedDate;
          }
        } else {
          // --- use as string
          previousValue[getIndexFieldName(currentValue.field)] = currentValue.value;
