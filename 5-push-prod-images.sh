...
#sk dev push-image --name grabbill/grabbill-website --registry docker.evos.tech --tag ${TAG_SUFFIX} $DRY_RUN
#sk dev push-image --name grabbill/grabbill-client --registry docker.evos.tech --tag ${TAG_SUFFIX} $DRY_RUN
#sk dev push-image --name grabbill/grabbill-admin --registry docker.evos.tech --tag ${TAG_SUFFIX} $DRY_RUN


sk dev push-image --name grabbill/grabbill-server --registry docker.evos.tech --tag hf1