...
@Data
@Entity
@Table(name = "mt_txe_record")
public class MTTransactionalEmailRecord extends BaseRecord {

    @Id
    @SequenceGenerator(name = "MT_TXE_REC_SEQ", sequenceName = "mt_txe_rec_id_seq", allocationSize = 1)
    @GeneratedValue(strategy = SEQUENCE, generator = "MT_TXE_REC_SEQ")
    private Long id;
    ...
    @OneToOne(mappedBy = "mtTransactionalEmailRecord")
    private MTTransactionalEmailIndexRow mtTransactionalEmailIndexRow;
