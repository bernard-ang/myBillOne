package com.grabbill.core.service;

import com.grabbill.core.entity.MTTransactionalEmailActivity;
import com.grabbill.core.entity.MTTransactionalEmailType;
import com.grabbill.core.entity.User;
import com.grabbill.core.entity.UserCode;
import com.grabbill.core.repository.MTTransactionalEmailTypeRepository;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;

import java.util.List;
import java.util.Optional;
import java.util.Set;
import java.util.stream.Collectors;

/**
 * @author michaellow
 */
public class MTTransactionalEmailTypeServiceImpl
        implements BaseTypeService<MTTransactionalEmailType, MTTransactionalEmailActivity> {

    @Autowired
    private MTTransactionalEmailTypeRepository repository;


    @Override
    public Page<MTTransactionalEmailType> getAll(
            final User user,
            final Pageable pageable
    ) {
        Set<String> codes = user.getUserCodes().stream().map(UserCode::getCode).collect(Collectors.toSet());
        if (codes.isEmpty()) {
            return repository.findByAccountId(user.getAccount().getId(), pageable);
        } else {
            return repository.findByAccountIdAndCodeIn(user.getAccount().getId(), codes, pageable);
        }
    }

    @Override
    public Page<MTTransactionalEmailType> getAllByName(
            final User user,
            final String name,
            final Pageable pageable
    ) {
        Set<String> codes = user.getUserCodes().stream().map(UserCode::getCode).collect(Collectors.toSet());
        if (codes.isEmpty()) {
            return repository.findByNameIsContainingIgnoreCaseAndAccountId(name, user.getAccount().getId(), pageable);
        } else {
            return repository.findByNameIsContainingIgnoreCaseAndAccountIdAndCodeIn(name, user.getAccount().getId(), codes, pageable);
        }
    }

    @Override
    public MTTransactionalEmailType getByActivity(
            final MTTransactionalEmailActivity mtTransactionalEmailActivity
    ) {
        // To not allow direct retrieve by ID
        return repository.getById(mtTransactionalEmailActivity.getMtTransactionalEmailType().getId());
    }

    @Override
    public Optional<MTTransactionalEmailType> getById(
            final User user,
            final Long id
    ) {
        return repository.findByIdAndAccountId(id, user.getAccount().getId());
    }

    @Override
    public Optional<MTTransactionalEmailType> getByName(
            final User user,
            final String name
    ) {
        return repository.findByNameAndAccountId(name, user.getAccount().getId());
    }

    @Override
    public MTTransactionalEmailType save(
            final MTTransactionalEmailType mtTransactionalEmailType
    ) {
        return repository.save(mtTransactionalEmailType);
    }

    @Override
    public void delete(
            final MTTransactionalEmailType mtTransactionalEmailType
    ) {
        repository.delete(mtTransactionalEmailType);
    }

    @Override
    public String findNextDuplicateName(
            final String mtTransactionalEmailName
    ) {
        String name = mtTransactionalEmailName.replaceFirst(" \\(copy \\d\\d\\)", "");
        List<MTTransactionalEmailType> types = repository.findByNameStartsWithOrderByNameAsc(name);
        int nextIndex = 1;
        for (MTTransactionalEmailType type : types) {
            if(type.getName().equals(name +  " (copy " + String.format("%02d", nextIndex) + ")")) {
                nextIndex++;
            }
        }
        return name +  " (copy " + String.format("%02d", nextIndex) + ")";
    }

}
