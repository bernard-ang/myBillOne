...
@Slf4j
@Transactional
public class MTTransactionalEmailActivitySftpServiceImpl
        implements BaseActivitySftpService<MTTransactionalEmailType, MTTransactionalEmailActivity, MTTransactionalEmailIndexField> {
    ...
    private BaseFileService<MTTransactionalEmailType, MTTransactionalEmailActivity, MTTransactionalEmailFile> mtTransactionalEmailFileService;
    ...
    public SftpFolderValidationSummary validate(
            ...
            final MTTransactionalEmailType type,
            final List<MTTransactionalEmailIndexField> indexFields,
            final MTTransactionalEmailActivity activity
    ...
    public void process(
        ...
        try {
            ...
            for (ChannelSftp.LsEntry entry : channelSftp.ls("./" + IN_FOLDER)) {
                ...
                if (!entry.getAttrs().isDir()) {
                    ...
                    if (entry.getFilename().endsWith(PDF_EXT)) {
                        ...
                        Optional<MTTransactionalEmailFile> fileOptional = mtTransactionalEmailFileService.getByNameAndActivity(
                        ...
                        if (fileOptional.isPresent()) {
                            ...
                            fileInstance.setMtTransactionalEmailType(type);
                            fileInstance.setMtTransactionalEmailActivity(activity);
                            mtTransactionalEmailFileService.save(fileInstance);
                            ...
                            newFileInstance.setMtTransactionalEmailType(type);
                            newFileInstance.setMtTransactionalEmailActivity(activity);
                            mtTransactionalEmailFileService.save(newFileInstance);
                        }
            ...
            List<MTTransactionalEmailIndexRow> targetIndexRows = activity.getMtTransactionalEmailIndexRows();
            ...
            for (; i < excelIndexRowList.size(); i++) {
                if (targetIndexRows.size() > i) {
                    ...
                    indexRow.setMtTransactionalEmailActivity(activity);
                    indexRow.setMtTransactionalEmailType(activity.getMtTransactionalEmailType());
                    excelIndexRowList.get(i).to(indexRow);
                    activity.getMtTransactionalEmailIndexRows().add(indexRow);
                }
            }
            ...
            for(; i < currentSize; i++) {
                activity.getMtTransactionalEmailIndexRows().remove(newSize);
            }
