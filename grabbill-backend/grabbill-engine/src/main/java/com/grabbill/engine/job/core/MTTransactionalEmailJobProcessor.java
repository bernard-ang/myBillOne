...
@Slf4j
@Transactional
public class MTTransactionalEmailJobProcessor extends AbstractJobProcessor {
    ...
    void processInternal(final Job targetJob) throws JobProcessingException {
        ...
        MTTransactionalEmailActivity targetActivity = mtTxeaService.markAsProcessing(targetJob.getActivityId());
        MTTransactionalEmailType targetType = targetActivity.getMtTransactionalEmailType();
        logProcessing(targetJob, targetType.getId());


        targetActivity = mtTxeaService.getById(targetJob.getActivityId()).get();
        targetType = targetActivity.getMtTransactionalEmailType();
        ...
        MTTransactionalEmailActivity updatedActivity = mtTxeaService.save(targetActivity);
        ...
        for (MTTransactionalEmailIndexRow indexRow : updatedActivity.getMtTransactionalEmailIndexRows()) {
            ...
            if (!indexRowIdToExceptionMap.isEmpty() && indexRowIdToExceptionMap.containsKey(indexRow.getId())) {
                ...
                if (template.isPresent()) {
                    ...
                    Map<String, String> parameterMap = indexRowHelper.buildMtTransactionalEmailParameterMap(indexRow, targetType.getMtTransactionalEmailIndexFields());
                    Map<String, MTTransactionalEmailFile> fileMap = targetActivity.getMtTransactionalEmailFiles().stream()
                            .collect(Collectors.toMap(MTTransactionalEmailFile::getName, Function.identity()));

                    processWhatsappMessage(record.getMtTransactionalEmailIndexRow(), record, targetActivity, targetType, parameterMap, fileMap, template.get());
                }
        ...
        if (!targetType.isArchive()) {
            ...
            for (MTTransactionalEmailFile targetFile : updatedActivity.getMtTransactionalEmailFiles()) {
            ...
            updatedActivity.getMtTransactionalEmailFiles().clear();
            updatedActivity = mtTxeaService.saveAndFlush(updatedActivity);
        ...
        MTTransactionalEmailType updatedType = mtTxetService.getByActivity(updatedActivity);
        ...
        for (MTTransactionalEmailIndexField targetIndexField : updatedType.getMtTransactionalEmailIndexFields()) {
            targetIndexField.setHardRef(true);
        }
        mtTxetService.save(updatedType);
    ...
    private Map<Long, Exception> processIndexRowsInternal(
            final MTTransactionalEmailType targetType,
        ...
        Map<String, MTTransactionalEmailActivityTemplate> transactionalEmailActivityTemplateMap = new HashMap<>();
        for (MTTransactionalEmailActivityTemplate mtTransactionalEmailActivityTemplate : targetActivity.getMtTransactionalEmailActivityTemplates()) {
            transactionalEmailActivityTemplateMap.put(mtTransactionalEmailActivityTemplate.getEmailTemplateName(), mtTransactionalEmailActivityTemplate);
        }

        List<String> rawEmailContents = transactionalEmailActivityTemplateMap.values().stream()
                .map(MTTransactionalEmailActivityTemplate::getEmailContent).collect(Collectors.toList());
        ...
        for (MTTransactionalEmailIndexRow indexRow : targetActivity.getMtTransactionalEmailIndexRows()) {
            ...
            MTTransactionalEmailActivityTemplate targetEmailTemplate =
                    transactionalEmailActivityTemplateMap.get(templateName);
            MTTransactionalEmailRecord record = indexRow.getMtTransactionalEmailRecord();

            // IMPORTANT: skipping those index row with record (already processed)
            if (record == null) {
                record = new MTTransactionalEmailRecord();
                record.setName(email);
                record.setPriority(targetActivity.getPriority());
                record.setMtTransactionalEmailActivity(targetActivity);
                record.setMtTransactionalEmailIndexRow(indexRow);
                ...
                try {
                    ...
                    // otherwise, process and send the email
                        ...
                        String rawEmailSubject = targetEmailTemplate.getEmailSubject();
                        ...
                        String rawEmailContent = targetEmailTemplate.getEmailContent();
                        ...
                        htmlDocument.select("a").forEach(element -> {
                            if (element.hasAttr("href")) {
                                ...
                                if (!hrefAttr.equals("{{unsubscribe_link}}") && isValidURL(hrefAttr)) {
                                    ...
                                    String urlPath = "mt-trxemail/" + targetActivity.getId()
                        ...
                        if (processedEmailContent.contains("</body>")) {
                            String emailOpenedImageTag = "<img id='grb' src='" + EMAIL_OPENED_LINK_URL + "/mt-trxemail/" + indexRow.getId() + "/pixel.png" + "'/></body>";
                        ...
                        if (targetType.isHasAttachment()) {
                            if (fileMap.containsKey(filename)) {
                                ...
                                MTTransactionalEmailFile file = fileMap.get(filename);
                                ...
                                indexRow.setMtTransactionalEmailFile(file);
                ...
                // link record to index rowc
                MTTransactionalEmailRecord savedRecord = mtTxerService.save(record);
                indexRow.setMtTransactionalEmailType(targetType);
                indexRow.setMtTransactionalEmailRecord(savedRecord);
                mtTxeirService.save(indexRow);
            }
        }
    ...
    private MTTransactionalEmailRecord processWhatsappMessage(
            MTTransactionalEmailIndexRow indexRow,
            MTTransactionalEmailRecord record,
            MTTransactionalEmailActivity targetActivity,
            MTTransactionalEmailType targetType,
            Map<String, String> parameterMap,
            Map<String, MTTransactionalEmailFile> fileMap,
        ...
        if (targetActivity.getSendWhatsAppMessage() && ProcessStatus.ERROR.equals(record.getStatus())) {
            ...
            for (MTTransactionalEmailWhatsappTemplateParam param : targetType.getMtTransactionalEmailWhatsappTemplateParams()) {
            ...
            MTTransactionalEmailRecord updatedRecord = mtTxerService.save(record);
            ...
            if (headerComponentOptional.isPresent() && fileMap.containsKey(filename)) {
                MTTransactionalEmailFile transactionalEmailFile = fileMap.get(filename);
    ...
    void purgeInternal(final Job targetJob) throws JobProcessingException {
        MTTransactionalEmailActivity targetActivity = mtTxeaService.getById(targetJob.getActivityId()).get();
    ...
    private Map<String, EmbeddedLink> generateUrlToEmbeddedLinkMap(
            final DomainType domainType,
            final Long targetTypeId,
            final Long targetActivityId,
            final List<String> rawEmailContents
    ) {
        Map<String, EmbeddedLink> urlToEmbeddedLinkMap = new HashMap<>();
        for (String rawEmailContent : rawEmailContents) {
            Jsoup.parse(rawEmailContent)
                    ...
                    .forEach(element -> {
                        if (element.hasAttr("href")) {
                            ...
                            if (!hrefAttr.equals("{{unsubscribe_link}}")
                                    && !(hrefAttr.startsWith("{{") && hrefAttr.endsWith("}}"))
                                    && !urlToEmbeddedLinkMap.containsKey(hrefAttr)) {
    ...
    void startPrepareInternal(final Job prepareJob) throws JobProcessingException {
        MTTransactionalEmailActivity targetActivity = (MTTransactionalEmailActivity) getBaseActivity(prepareJob.getActivityId());
        MTTransactionalEmailType targetType = targetActivity.getMtTransactionalEmailType();
        Account account = targetType.getAccount();

        try {
            baseActivitySftpService.process(account, targetType, targetType.getMtTransactionalEmailIndexFields(), targetActivity);
            mtTxeaService.saveAndFlush(targetActivity);
        } catch (Exception e) {
    ...
    BaseActivity getBaseActivity(final long id) {
        return mtTxeaService.getById(id).get();
    }
