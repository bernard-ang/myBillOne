 ...
 * @author seez
 */
public class WhatsAppReportServiceImpl extends BaseReportService implements WhatsAppReportService {
        ...
        for (ReportType reportType : reportTypes) {
            if (ReportType.ACTIVITY_SUMMARY.equals(reportType)) {
                ...
                for (WhatsAppActivity activity : activities) {
                    ...
                    activitySummaryRow.setTotalWhatsApp(activity.getWhatsAppIndexRows().size());
                ...
                for (WhatsAppActivity activity : activities) {
                    WhatsAppType type = activity.getWhatsAppType();
                    List<WhatsAppIndexField> indexFields = type.getWhatsAppIndexFields();

                    List<ActivityRecordRow> activityRecordRows = new ArrayList<>();
                    for (WhatsAppRecord record : activity.getWhatsAppRecords()) {
                        WhatsAppIndexRow indexRow = record.getWhatsAppIndexRow();
    ...
    protected void buildActivitySummarySheet(
        ...
        headerCell.setCellValue("Template Body");
        headerCell.setCellStyle(headerCellStyle);

        headerCell = hearderRow.createCell(8);
        headerCell.setCellValue("Template Document");
        headerCell.setCellStyle(headerCellStyle);

        headerCell = hearderRow.createCell(9);
        headerCell.setCellValue("Template Footer");
        headerCell.setCellStyle(headerCellStyle);

        headerCell = hearderRow.createCell(10);
        headerCell.setCellValue("Template Button");
        headerCell.setCellStyle(headerCellStyle);
