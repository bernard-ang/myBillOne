 ...
 * @author michaellow
 */
public class TransactionalEmailReportServiceImpl extends BaseReportService implements TransactionalEmailReportService {
    ...
    @Override
    public Workbook generateTransactionalEmailReport(
        ...
        for (ReportType reportType : reportTypes) {
            if (ReportType.ACTIVITY_SUMMARY.equals(reportType)) {
                ...
                for (TransactionalEmailActivity activity : activities) {
                    TransactionalEmailType type = activity.getTransactionalEmailType();
                    ...
                    activitySummaryRow.setTotalEmails(activity.getTransactionalEmailIndexRows().size());
                ...
                for (TransactionalEmailActivity activity : activities) {
                    TransactionalEmailType type = activity.getTransactionalEmailType();
                    List<TransactionalEmailIndexField> indexFields = type.getTransactionalEmailIndexFields();

                    List<ActivityRecordRow> activityRecordRows = new ArrayList<>();
                    for (TransactionalEmailRecord record : activity.getTransactionalEmailRecords()) {
                        TransactionalEmailIndexRow indexRow = record.getTransactionalEmailIndexRow();
