 ...
 */
@Slf4j
public class EmailCampaignReportServiceImpl extends BaseReportService implements EmailCampaignReportService {
    ...
    @Override
    public Workbook generateEmailCampaignReport(
        ...
        for (ReportType reportType : reportTypes) {
            if (ReportType.ACTIVITY_SUMMARY.equals(reportType)) {
                ...
                for (EmailCampaignActivity activity : activities) {
                    EmailCampaignType type = activity.getEmailCampaignType();
                    ...
                    activitySummaryRow.setTotalEmails(activity.getEmailCampaignIndexRows().size());
                ...
                buildActivitySummarySheet(workbook, activitySummaryRows, false);
                ...
                for (EmailCampaignActivity activity : activities) {
                    EmailCampaignType type = activity.getEmailCampaignType();
                    List<EmailCampaignIndexField> indexFields = activity.getEmailCampaignIndexFields();

                    List<ActivityRecordRow> activityRecordRows = new ArrayList<>();
                    for (EmailCampaignRecord record : activity.getEmailCampaignRecords()) {
                        EmailCampaignIndexRow indexRow = record.getEmailCampaignIndexRow();
                ...
                buildActivityRecordsSheet(workbook, dynamicHeaders, allRecordRows, false);
                ...
                for (EmailCampaignActivity activity : activities) {
                    ...
                    EmailCampaignType type = activity.getEmailCampaignType();
                    for (EmbeddedLink embeddedLink : embeddedLinkService.getByDomainTypeAndActivityIdIs(DomainType.EMAIL_CAMPAIGN, activity.getId())) {
